...
const CorrectPwd = (rule, value, callback) => {
    if (JSON.parse(sessionStorage.getItem("userInfo")).password !== value) {
...
const rules = ref({
    oldPassword: [{ required: true, message: "旧密码不能为空", trigger: "blur" },
        { required: true, validator: CorrectPwd, trigger: "blur" }],
...
const handleSubmit=()=>{
    pwdRef.value.validate(async (valid)=>{
        if(valid) {
            let result = await requestUtil.post("info/user/resetPwd", form.value);
